...
| 文件 | 说明 |
|------|------|
| `src/mym2/services/report_service.py` | 统一 ReportService/query 入口，覆盖收支按分类、月度收支、账户余额/资产负债、应收汇总、预算执行、交易明细导出；提供 CSV/Excel/PDF 导出 |

**UI 页面：**
| 文件 | 说明 |
|------|------|
| `src/mym2/ui/pages/reports_page.py` | 报表中心：日期区间、账户/分类筛选、六类报表、口径说明、CSV/Excel/PDF 按钮 |
| `src/mym2/ui/widgets/chart_web_view.py` | offscreen 测试环境使用轻量降级控件，正常 GUI 仍使用 QWebEngineView + 本地 ECharts |

**测试：**
| 文件 | 说明 |
|------|------|
| `tests/test_report_center.py` | 7 个报表验收测试：统一口径、仪表盘一致性、导出日期/金额/UTF-8、离线 PDF、历史投资快照不变成功能性股票模块 |

### 功能实现

- 统一 `ReportService.query()`，所有报表与仪表盘关键总额共享同一口径。
- 日常消费/收入默认只统计 `expense` / `income`，排除 `balance_adjustment`、`historical_investment_settlement`、应收垫付/还款本金。
- 账户余额/资产负债报表包含不可编辑历史投资资产快照，但只作为历史估值计入净资产。
- 应收汇总展示区间垫付、区间收回与当前待收余额。
- 预算执行仅统计已分类 `expense`。
- 交易明细可按日期、账户、分类筛选并导出 CSV/Excel；CSV 使用 UTF-8 BOM 并做公式注入防护。
- PDF 打印使用 Qt `QTextDocument` + `QPdfWriter`，生成 HTML 不依赖互联网/CDN。
- 报表页面明确显示口径说明，避免把历史估值或应收本金理解为本月日常收支。

### 验收结果

- ✅ `.venv/bin/python -m pytest` — **420 passed**
- ✅ `.venv/bin/ruff check .` — All checks passed
- ✅ 同一区间仪表盘与报表关键总额一致
- ✅ CSV/Excel 导出日期、金额、UTF-8 中文与公式注入防护正确
- ✅ PDF 打印 HTML 不含 `http://`、`https://`、CDN，PDF 可离线生成
- ✅ 历史投资快照仅显示为不可编辑历史资产估值，不提供持仓/行情/买卖等股票功能
